"""Example: Strands Agents SDK with SpecOps tracing (Anthropic).
...
Mock mode (no API key or strands needed):
    SPECOPS_EXAMPLE_MODE=mock uv run examples/providers/anthropic/strands_agent.py
"""
...
@trace_agent(name="strands-anthropic-agent", framework="strands")
def run_strands_agent(prompt: str) -> str:
    """Run a Strands agent with Anthropic backend."""
    if os.environ.get("SPECOPS_EXAMPLE_MODE") == "mock":
...
if __name__ == "__main__":
    print("=" * 60)
    print("SpecOps AI — Strands Agent Example (Anthropic)")
    print("=" * 60)
