"""Example: Strands Agents SDK with SpecOps tracing (Grok/xAI).
...
api_key = require_api_key("GROK_API_KEY", "Grok")
...
@trace_agent(name="strands-grok-agent", framework="strands")
def run_strands_agent(prompt: str) -> str:
    """Run a Strands agent with Grok/xAI backend."""
    if os.environ.get("SPECOPS_EXAMPLE_MODE") == "mock":
    ...
    model = OpenAIModel(
        client_args={"api_key": api_key, "base_url": "https://api.x.ai/v1"},
        model_id=get_model("grok"),
    )
...
if __name__ == "__main__":
    print("=" * 60)
    print("SpecOps AI — Strands Agent Example (Grok/xAI)")
    print("=" * 60)
